...
import { SEOAwith4337, SEOAwith4337__factory } from "../typechain/index.js";
...
const owner = new ethers.Wallet(
  "5e1e9d81398b795ae134ac8c85eb6743260175099aa094ad836340fd65387c88",
  deployer.provider,
);
  ...
  if (ownerBalance !== undefined && ownerBalance < ethers.parseEther("1")) {
    await (
      await donor.sendTransaction({
        to: owner.address,
        value: ethers.parseEther("1") - ownerBalance,
      })
    ).wait();
  }
};
...
const main = async () => {
  ...
  if (!hasDelegation) {
    const sEOAImplementation = await (
      await new SEOAwith4337__factory(owner).deploy()
    ).waitForDeployment();
    ...
    const tx = await owner.sendTransaction({
      ...
      data: SEOAwith4337__factory.createInterface().encodeFunctionData(
        "grantRole",
    ...
    sEOA = SEOAwith4337__factory.connect(owner.address, owner);
  } else {
...
const createUserOp = async (sEOA: SEOAwith4337) => {
  ...
  const calldata = SEOAwith4337__factory.createInterface().encodeFunctionData(
    "execute",
    [mode, encodedBatch],
  );
  // const nonceKey = // TODO
...
const sendUserOpToEntryPoint = async (
  packedUserOps: PackedUserOp[],
  sender: Signer,
) => {
  const entryPointContract = EntryPoint__factory.connect(ENTRYPOINT_V7, sender);

  await (
    await owner.sendTransaction({ to: ENTRYPOINT_V7, value: parseEther("0.5") })
  ).wait();

...
use crate::queue::{ReceiptPollerEvent, ReceiptPollerQueueMessage, ReceiptPollerQueueMessageBody};

pub struct ReceiptPollerSqsQueue {
    pub client: aws_sdk_sqs::Client,
    pub queue_url: String,
    pub message_group_id: String,
}

impl ReceiptPollerSqsQueue {
    pub fn build(
    ...
    pub async fn send_new(
        &self,
        message_body: &ReceiptPollerQueueMessageBody,
    ) -> anyhow::Result<()> {
...
impl ReceiptPollerQueueMessageBody {
    pub fn from_sqs_message(message: &SqsMessage) -> anyhow::Result<Option<Self>> {
        ...
        let queue_message_body =
            match serde_json::from_str::<ReceiptPollerQueueMessageBody>(&body).ok() {
                Some(v) => v,
                None => {
...
impl ReceiptPollerEvent {
    pub fn from_sqs_event(event: LambdaEvent<SqsEvent>) -> anyhow::Result<ReceiptPollerEvent> {
        let mut messages = Vec::new();

        for record in event.payload.records {
            let Some(message_body) = ReceiptPollerQueueMessageBody::from_sqs_message(&record)?
            else {
                continue;
            };
            ...
            messages.push(ReceiptPollerQueueMessage {
                message_id,
                body: message_body,
            });
        }
